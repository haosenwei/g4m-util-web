...
function acePostMain(title , doUrl,tableName,min,max,title,refresh,name){
	var values = getCheckBoxValues(tableName);
	if(values.length > max){
		showTips("最多选择"+max+"条数据!");
		return false;
	}
	if(values.length < min){
		showTips("最少选择"+min+"条数据!");
		return false;
	}
	acePost(doUrl, {"ids" :values.join(',')}, title, refresh, name);
}
	...
	'<div class="hidden-md hidden-lg">'+
		'<div class="inline pos-rel">'+
			...
			'<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">'+
				...
				'<li><a href="javascript:void(0);" class="tooltip-info" data-rel="tooltip" onclick="aceViewf(\''+cellData+'\')" title="查看" data-original-title="View"><span class="blue"><i class="ace-icon fa fa-search-plus bigger-120"></i></span></a></li>'+
